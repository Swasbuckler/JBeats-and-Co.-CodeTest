// This JavaScript file consist of the variables, classes and functions for the physics engine
...
class Vector { // Vector Calculations
    constructor(x, y) {
        this.x = x;
        this.y = y;
    }
...
class VectorMath { // Advanced Vector Calculations
...
class Body { // Logic for the objects in the physics engine. Which is either Circles or Boxes
...
class Transform { // Transform object which is used for rotation
...
class Collision { // Collision logic for the objects
...
class AABB { // The min and max points in a axis of an object
    constructor(min, max) {
        this.min = min;
        this.max = max;
    }
...
class Manifold { // A compilation of variables that is used to make the code not as dense
...
class Level { // Sections in a page that consists of objects 
    constructor(height) {
        this.height = height;
        this.bodyList = [];
    }
...
function stepBodies(time, iterations, colourShift) { // Function to calculate the movement of the objects in the next frame
...
function seperateBodies(bodyA, bodyB, mtv) { // Function to seperates the objects if they are inside each other in the frame
    if (bodyA.isStatic) {
        bodyB.move(mtv);
...
function broadPhase() { // Function that checks if objects are near enough to each other that could mea a collision in the frame
...
function narrowPhase(colourShift) { // Function to check if the near objects are colliding with each other
...
function step(time, iterations, colourShift) { // A Function that runs all of the functions before it 
...
// User Controls : Checks where the mouse is in the canvas
